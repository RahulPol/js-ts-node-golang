...
npx create-react-app my-react-app --template typescript <-- this for react with ts

### Vite

npm create vite@latest mixmaster -- --template react-ts <-- this for react with ts
...
- always close tag <Greeting/> when using this component
...
const Book = () => {
  ...
  return (
    <article className="book">
      ...
      {/* <p>{let x = 6}</p> */} <-- this is an error as its a statement and not expression.
      <p>{6 + 6}</p>
    </article>
  );
};
...
const Book = (props) => {
  ...
  return (
    <article className="book">
      ...
      <h2>{title}</h2>
      <h4>{author} </h4>
      <p>{props.job}</p> <-- for second book, this will be blank
      <p>{props.title}</p> <-- for first book, this will be blank
      <p>{props.number}</p> <-- for first book, this will be blank
    </article>
  );
};
```
...
- setup an object (firstBook)
- refactor vars to properties (firstBook.author, firstBook.title, etc.)
- copy/paste and rename
- get values for second book (secondBook)
- setup props
...
function BookList() {
  return <section className="booklist">{names}</section>; <-- this is ok
}
```
...
function BookList() {
  return <section className="booklist">{newNames}</section>;  <-- this will give error for "key" prop.
}
```
...
- you will use index of map function, but it's not advised if the list is changing
- more info: https://medium.com/@sriweb/avoid-using-array-index-as-a-key-in-react-components-1978799ed94b#:~:text=In%20scenarios%20where%20items%20have,indexes%20are%20used%20as%20keys.
...
const EventExamples = () => {
  ...
  return (
    <section>
      <button onClick={handleButtonClick}>click me</button>  <-- do not include () in function name for onClick
    </section>
  );
};
```
...
const EventExamples = () => {
  ...
  return (
    <section>
      ...
      <form onSubmit={handleFormSubmission}> <-- in typescript it works differently, you need to provide event object
        <h2>Typical Form</h2>
        <input
          type="text"
          name="example"
...
- components are independent by default: every instance of book will create separate html dom and event handler
- it is not by default in vanilla js. In vanilla js you need get the correct book using selector and then apply logic
  However in react by default every instance have its own js copy.
...
const Book = (props) => {
  ...
  return (
    <article className="book">
      ...
      <button onClick={getBook(id)}>display title</button> <-- this is not going to work
      <h4>{author}</h4>
    </article>
  );
};
...
- why react images in src folder requires import?
  - If you include images as part of public folder the images are hosted by the server, however when you include image in src folder,
