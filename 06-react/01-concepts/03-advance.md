...
- [Udemy] (https://xebia.udemy.com/course/react-tutorial-and-projects-course/)
...
- traditional Vite app
  ...
  - removed <StrictMode>, so it's less logs. If you keep strict mode you will see 2 logs for every console.log statement
...
const UseStateBasics = () => {
  ...
  // console.log(useState('jo koy')); <-- ["jo koy", function(){}]
...
- hooks must be called at the top level of a function component or custom hook and not inside nested functions or conditional statements
  - This restriction is imposed by the rules of the React Hooks API
...
- cb can't return promise (so can't make it async)
  - Reason: In React, the useEffect hook is designed to handle side effects in functional components. The useEffect function itself cannot be marked as async, but you can work with asynchronous code inside the callback function passed to useEffect.
...
const FetchData = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      // writing try...catch is best practice with async await
      try {
        ...
        // Gotcha: in case of fetch API 4xx / 5xx is not an error, so check the ok flag of response or use Axios instead of fetch
        console.log(error);
      }
    };
    fetchData();
...
const RandomComponent = () => {
  useEffect(() => {
    ...
    // thus clear interval in cleanup function
    // the cleanup function will run after render of parent before any useEffect of parent
...
const MultipleReturnsFetchData = () => {
  ...
  // NOTE: instead of simply returning user JSX, we can do a check {user && <User JSX>} that way we don't need to worry about user being undefined.
  // It can be further simplified by adding {isLoading && <Loading JSX>} and {isError && <Error JSX>}
  if (isLoading) {
    return <h2>Loading...</h2>;
  }
  if (isError) {
...
Unlike Axios, by default, the fetch() API does not consider HTTP status codes in the 4xx or 5xx range to be errors. Instead, it considers these status codes to be indicative of a successful request,
...
In React, the terms "controlled" and "uncontrolled" refer to how you manage the state of form elements such as input fields, checkboxes, and radio buttons within your components.

Controlled Inputs:
In a controlled input, the React component's state is used to control and manage the value of the input element. The value of the input is stored in the component's state, and the component re-renders whenever the state changes.

Uncontrolled Inputs:
In an uncontrolled input, the React component does not directly manage the state of the input. Instead, the input element holds its own state, and you access its value through refs or other means.

Choosing Between Controlled and Uncontrolled Inputs:

Controlled Inputs:
Provide more control over the input state.
Useful when you need to manipulate or validate the input value before rendering.
State changes trigger re-renders, allowing you to easily track and manage the input's value.

Uncontrolled Inputs:
Can be simpler for certain scenarios, especially when integrating with non-React code or libraries.
Useful when you want to delegate the state management to the DOM.
May be appropriate for performance optimizations in specific situations.
...
- Gets rid of prop drilling
- context is created using createContext(), provides 2 important returns
  - Provider
  - Consumer
- Use provider to wrap the parent component with value to passed in children
- Use consumer(created through useContext hook) in any child component to access the value
- The creation of context, wrapping of provider and creation of consumer is all done in parent component only.
...
- can be thought of as light version or redux
- It is an alternative or middle ground between to the more commonly used useState Hook and full blown state management library like Redux
