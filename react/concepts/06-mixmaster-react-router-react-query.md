...
The about setup won't work as it is, you need to include Outlet component in the HomeLayout page. Now, the HomeLayout has become a shared layout and whatever you add in this page will be visible in all pages.
In the following case `<nav> navbar </nav>` is available in all paths, for eg. /about, /landing, /cocktail
...
add `index: true` property to the page which you want to be the default page when `/` route is visited. In the following case `<Landing />` page is made default page whenever you visit `/`.
...
Lets say you want to setup nested pages like `/about/company` and `/about/people`. You can do it as follows,
...
Note: Now about also become a shared layout for company and people.
...
```js
import { NavLink } from 'react-router-dom'; <-- notice instead of link we are using NavLink
...
Now, instead of adding `Navbar` for each page, we can add it in shared layout page, `HomeLayout.jsx`
...
React router provide out of the box feature for error handling with `errorElement`, you can setup `errorElement` for any pages but if you don't provide it then the error will bubble up till root path, so you must setup errorElement on at least root path.
...
const router = createBrowserRouter([
  {
    path: '/',
    element: <HomeLayout />,
    errorElement: <Error />, <-- mention error page here
    children: [
      {
        index: true,
        element: <Landing />,
...
import { Link, useRouteError } from 'react-router-dom'; < -- Note useRouterError
...
The problem with above approach(useEffect) is that the data will be loaded after page load is complete. However in the newer version or react router this issue is resolved

#### Loader

Loader is essentially a function that provide data to our route before it renders(depending on data size and network speed) i.e. prefetching.
Each route can define a "loader" function to provide data to the route element before it renders.

- must return something even "null" otherwise error <-- Important
